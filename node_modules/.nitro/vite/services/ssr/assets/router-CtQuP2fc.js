...
import { useQuery, useQueryClient, QueryClientProvider, useSuspenseQuery, queryOptions, QueryClient } from "@tanstack/react-query";
import { useRouter, isRedirect, useRouterState, useNavigate, createRootRouteWithContext, redirect, Link, Outlet, HeadContent, Scripts, createFileRoute, lazyRouteComponent, notFound, createRouter } from "@tanstack/react-router";
...
import { r as requireAdmin, a as requireAuth } from "./auth-middleware.server-B9hAjfqi.js";
import { z } from "zod";
import { X, MessageCircle, Headphones, FileUp, CheckCircle2, PowerOff, Send } from "lucide-react";
import { markExpired, findExpiringSoon } from "./vip.repo-CycBrLVA.js";
import { S as SAUDI_CITIES } from "./saudi-cities-D2sGDQV3.js";
import { d as db, r as rowsToObjects, g as getSessionClaims } from "./db-D5OYORU-.js";
import { S3Client, ListObjectsV2Command } from "@aws-sdk/client-s3";
import { makeKey, uploadToR2, signGetUrl, getBucket } from "./r2-CJ2zxhhj.js";
import { verifyWebhookRequest, WebhookError } from "@lovable.dev/webhooks-js";
import { Html, Head, Preview, Body, Container, Heading, Text, render } from "@react-email/components";
...
const startVisitorChat = createServerFn({
  ...
  visitorToken: uuid,
  visitorName: z.string().trim().max(80).nullable().optional()
...
const visitorSendMessage = createServerFn({
  ...
  qaId: z.preprocess((v) => v == null || v === "" ? null : String(v), z.string().nullable()).optional()
...
const updateBotSettings = createServerFn({
  ...
  off_hours_message: z.string().trim().min(1).max(1e3),
  fallback_message: z.string().trim().min(1).max(1e3),
  allow_escalation: z.boolean(),
  show_suggested_questions: z.boolean(),
  local_enabled: z.boolean(),
  local_system_prompt: z.string().trim().max(4e3)
...
const addProjectSchema = z.object({
  ...
  file_base64: z.string().min(8).max(15e6)
});
createServerFn({
  method: "POST"
...
const adminUpdateOfferStatus = createServerFn({
  ...
  id: z.string().uuid(),
  status: z.enum(["pending", "new", "reviewing", "accepted", "rejected"])
...
async function runVipExpiryCheckRaw() {
  ...
  const {
    sendResendEmail
  } = await import("./resend-send.server-Cc6n_-h6.js");
...
const adminReplyContactMessage = createServerFn({
  ...
  id: z.string().uuid(),
  reply: z.string().trim().min(1).max(5e3)
...
const sendRequestMessage = createServerFn({
  ...
  to: z.string().trim().email().max(255),
  message: z.string().trim().min(1).max(3e3)
...
createServerFn({
  ...
  projectId: z.string().uuid(),
  durationHours: z.number().int().min(0).max(720)
...
function SupportChatWidget() {
  ...
  const [offerMsgId, setOfferMsgId] = useState(null);
  const [offerStep, setOfferStep] = useState(null);
  ...
  const [vipMsgId, setVipMsgId] = useState(null);
  const [vipStep, setVipStep] = useState(null);
  ...
  return /* @__PURE__ */ jsxs(Fragment, { children: [
    !open && /* @__PURE__ */ jsxs("div", { className: "fixed bottom-5 right-5 z-50 flex flex-col items-end gap-2", children: [
      showBubble && /* @__PURE__ */ jsxs(
        "button",
        {
          ...
          children: [
            /* @__PURE__ */ jsx("span", { className: "block pr-5", children: "تحتاج مساعدة؟ 👋" }),
            /* @__PURE__ */ jsx(
              "span",
              {
                onClick: (e) => {
    ...
    open && /* @__PURE__ */ jsxs(
      "div",
      {
        ...
        children: [
          /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-between border-b border-border bg-[image:var(--gradient-hero)] px-4 py-3 text-primary-foreground", children: [
            /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-2", children: [
              /* @__PURE__ */ jsx("span", { className: "grid h-8 w-8 place-items-center rounded-full bg-white/15", children: /* @__PURE__ */ jsx(Headphones, { className: "h-4 w-4" }) }),
              /* @__PURE__ */ jsxs("div", { children: [
                /* @__PURE__ */ jsx("div", { className: "text-sm font-bold", children: "دعم العمران" }),
                /* @__PURE__ */ jsx("div", { className: "text-[11px] opacity-80", children: status === "escalated" ? "متصل مع موظف" : status === "closed" ? "المحادثة مغلقة" : "المساعد الآلي" })
              ] })
            ] }),
            /* @__PURE__ */ jsx("button", { onClick: () => setOpen(false), className: "rounded-md p-1 hover:bg-white/10", "aria-label": "إغلاق", children: /* @__PURE__ */ jsx(X, { className: "h-5 w-5" }) })
          ] }),
          /* @__PURE__ */ jsxs("div", { ref: scrollRef, className: "flex-1 space-y-2 overflow-y-auto bg-secondary/30 p-3", children: [
            messages.map((m) => {
              const mine = m.sender === "visitor";
              const isSystem = m.sender === "system";
              if (isSystem) {
                return /* @__PURE__ */ jsx("div", { className: "mx-auto max-w-[85%] rounded-md bg-accent/15 px-3 py-1.5 text-center text-[11px] text-foreground/70", children: m.body }, m.id);
              }
              return /* @__PURE__ */ jsx("div", { className: `flex ${mine ? "justify-end" : "justify-start"}`, children: /* @__PURE__ */ jsxs("div", { className: `max-w-[85%] rounded-2xl px-3 py-2 text-sm shadow-sm ${mine ? "bg-primary text-primary-foreground" : m.sender === "admin" ? "bg-accent text-accent-foreground" : "bg-background border border-border"}`, children: [
                m.sender === "admin" && /* @__PURE__ */ jsx("div", { className: "mb-0.5 text-[10px] font-semibold opacity-80", children: "موظف الدعم" }),
                /* @__PURE__ */ jsx("div", { className: "whitespace-pre-wrap break-words", dangerouslySetInnerHTML: { __html: m.body.replace(OFFER_FLOW_MARKER, "").replace(VIP_FLOW_MARKER, "").trim() } })
              ] }) }, m.id);
            }),
            offerStep === "terms" && /* @__PURE__ */ jsx("div", { className: "rounded-xl border border-border bg-background p-3", children: /* @__PURE__ */ jsx(
              "button",
              {
            ...
            offerStep === "form" && /* @__PURE__ */ jsxs("div", { className: "space-y-2 rounded-xl border border-border bg-background p-3", children: [
              /* @__PURE__ */ jsx("div", { className: "text-[11px] font-semibold text-muted-foreground", children: "بيانات عرض السعر:" }),
              /* @__PURE__ */ jsxs(
                "select",
                {
                  ...
                  children: [
                    /* @__PURE__ */ jsx("option", { value: "", children: "اختر المشروع" }),
                    projectsList.map((p) => /* @__PURE__ */ jsx("option", { value: p.id, children: p.name }, p.id))
                  ]
                }
              ),
              [
              ...
              /* @__PURE__ */ jsxs("label", { className: "flex cursor-pointer items-center justify-center gap-1.5 rounded-md border border-dashed border-border bg-secondary/40 px-3 py-2 text-[11px] font-medium hover:bg-secondary", children: [
              ...
              offerError && /* @__PURE__ */ jsx("div", { className: "text-[11px] text-destructive", children: offerError }),
            ...
            offerStep === "done" && /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-center gap-1.5 rounded-xl border border-border bg-background p-3 text-[11px] font-semibold text-foreground/80", children: [
              /* @__PURE__ */ jsx(CheckCircle2, { className: "h-4 w-4 text-primary" }),
              "تم استلام عرضك بنجاح. سيتم اشعاركم بأي تحديث"
            ] }),
            vipStep === "terms" && /* @__PURE__ */ jsxs("div", { className: "rounded-xl border border-border bg-background p-3", children: [
              /* @__PURE__ */ jsxs("div", { className: "mb-2 space-y-1 text-[11px] text-muted-foreground", children: [
                VIP_PLANS.map((p) => /* @__PURE__ */ jsx("div", { className: "font-semibold text-foreground", children: p.label }, p.value)),
                /* @__PURE__ */ jsx("div", { className: "pt-1", children: "تستقبل مشاريع خاصة عبر الإيميل بلا منافس + دعم فني VIP" }),
                /* @__PURE__ */ jsx("div", { className: "pt-1", children: "IBAN: SA35 1000 0065 5000 4711 0807" })
              ] }),
              /* @__PURE__ */ jsx(
                "button",
                {
            ...
            vipStep === "form" && /* @__PURE__ */ jsxs("div", { className: "space-y-2 rounded-xl border border-border bg-background p-3", children: [
              /* @__PURE__ */ jsx("div", { className: "text-[11px] font-semibold text-muted-foreground", children: "بيانات الاشتراك:" }),
              /* @__PURE__ */ jsx(
                "input",
                {
                  value: vipForm.name,
              ...
              /* @__PURE__ */ jsxs(
                "select",
                {
                  ...
                  className: "w-full rounded-md border border-border bg-background px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-ring",
                  children: [
                    /* @__PURE__ */ jsx("option", { value: "", children: "اختر المدينة" }),
                    SAUDI_CITIES.map((c) => /* @__PURE__ */ jsx("option", { value: c, children: c }, c))
                  ]
                }
              ),
              /* @__PURE__ */ jsxs(
                "select",
                {
                  ...
                  className: "w-full rounded-md border border-border bg-background px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-ring",
                  children: [
                    /* @__PURE__ */ jsx("option", { value: "", children: "اختر الباقة" }),
                    VIP_PLANS.map((p) => /* @__PURE__ */ jsx("option", { value: p.value, children: p.label }, p.value))
                  ]
                }
              ),
              /* @__PURE__ */ jsxs("label", { className: "flex cursor-pointer items-center justify-center gap-1.5 rounded-md border border-dashed border-border bg-secondary/40 px-3 py-2 text-[11px] font-medium hover:bg-secondary", children: [
              ...
              vipError && /* @__PURE__ */ jsx("div", { className: "text-[11px] text-destructive", children: vipError }),
              /* @__PURE__ */ jsxs("div", { className: "flex gap-2", children: [
                /* @__PURE__ */ jsx(
                  "button",
                  {
            ...
            vipStep === "done" && /* @__PURE__ */ jsxs("div", { className: "flex items-center justify-center gap-1.5 rounded-xl border border-border bg-background p-3 text-[11px] font-semibold text-foreground/80", children: [
              /* @__PURE__ */ jsx(CheckCircle2, { className: "h-4 w-4 text-primary" }),
              "تم استلام طلبك. سيتم إرسال تأكيد على إيميلك بعد الموافقة"
            ] }),
            showEndAfterBot && token && !offerStep && !vipStep && /* @__PURE__ */ jsx("div", { className: "flex justify-start pt-1", children: /* @__PURE__ */ jsxs(
              "button",
              {
                onClick: () => endSession(),
                ...
                children: [
                  /* @__PURE__ */ jsx(PowerOff, { className: "h-3 w-3" }),
                  "إنهاء المحادثة"
                ]
              }
            ) })
          ] }),
          canShowQuickQuestions && /* @__PURE__ */ jsxs("div", { className: "border-t border-border bg-background/60 p-2", children: [
            /* @__PURE__ */ jsx("div", { className: "mb-1 text-[11px] font-semibold text-muted-foreground", children: "اختر سؤالًا:" }),
            /* @__PURE__ */ jsx("div", { className: "flex flex-wrap gap-1.5", children: qaList.map((q) => /* @__PURE__ */ jsx(
              "button",
              {
          ...
          /* @__PURE__ */ jsxs("div", { className: "border-t border-border bg-background p-2", children: [
            /* @__PURE__ */ jsxs(
              "form",
              {
                onSubmit: (e) => {
            ...
            sendError && /* @__PURE__ */ jsx("div", { className: "mt-1 text-[11px] text-destructive", children: sendError })
          ] })
        ]
      }
    )
...
function RootComponent() {
  ...
  return /* @__PURE__ */ jsxs(QueryClientProvider, { client: queryClient, children: [
    /* @__PURE__ */ jsx(MaintenanceGate, {}),
    /* @__PURE__ */ jsx(Outlet, {}),
    /* @__PURE__ */ jsx(PublicSupportWidget, {})
  ] });
}
function PublicSupportWidget() {
  ...
  return /* @__PURE__ */ jsx(SupportChatWidget, {});
}
const projectsQuery = queryOptions({
  queryKey: ["projects"],
  queryFn: () => listProjects()
});
const $$splitComponentImporter$A = () => import("./index-CoHBLCUe.js");
...
const $$splitComponentImporter$z = () => import("./route-BFsOu0JM.js");
...
const $$splitComponentImporter$y = () => import("./ads-kEBIZL7a.js");
const Route$K = createFileRoute("/ads")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$x = () => import("./auth-CoL-b1Fv.js");
const Route$J = createFileRoute("/auth")({
  component: lazyRouteComponent($$splitComponentImporter$x, "component")
});
const $$splitComponentImporter$w = () => import("./contact-Cc9JOcrY.js");
...
const $$splitComponentImporter$v = () => import("./forgot-password-CkJDHMm7.js");
const Route$H = createFileRoute("/forgot-password")({
  component: lazyRouteComponent($$splitComponentImporter$v, "component")
});
const $$splitComponentImporter$u = () => import("./maintenance-SSFEBgct.js");
const Route$G = createFileRoute("/maintenance")({
...
const $$splitComponentImporter$t = () => import("./my-requests-B7LzAKof.js");
const Route$F = createFileRoute("/my-requests")({
  component: lazyRouteComponent($$splitComponentImporter$t, "component")
});
const $$splitComponentImporter$s = () => import("./projects-FPGxDpVY.js");
const Route$E = createFileRoute("/projects")({
  component: lazyRouteComponent($$splitComponentImporter$s, "component")
});
const $$splitComponentImporter$r = () => import("./reset-password-DFACgX69.js");
...
const $$splitComponentImporter$q = () => import("./subscribe-success-D46myBHO.js");
const Route$C = createFileRoute("/subscribe-success")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$p = () => import("./thank-you-RtVEqYBZ.js");
...
const $$splitComponentImporter$o = () => import("./unsubscribe-CAo86eIt.js");
const Route$A = createFileRoute("/unsubscribe")({
  component: lazyRouteComponent($$splitComponentImporter$o, "component")
});
const $$splitComponentImporter$n = () => import("./vip-BFsOu0JM.js");
const Route$z = createFileRoute("/vip")({
  component: lazyRouteComponent($$splitComponentImporter$n, "component")
});
const $$splitComponentImporter$m = () => import("./admin-BezxMKEs.js");
const Route$y = createFileRoute("/_authenticated/admin")({
...
const Route$w = createFileRoute("/_authenticated/dashboard")({
  beforeLoad: () => {
    throw redirect({ to: "/admin/requests" });
  }
});
const Route$v = createFileRoute("/_authenticated/employees")({
  beforeLoad: () => {
    throw redirect({ to: "/admin/employees" });
  }
});
const $$splitComponentImporter$l = () => import("./upload-CgTPINYJ.js");
const Route$u = createFileRoute("/_authenticated/upload")({
...
const adSchema = z.object({
  ...
  description: z.string().trim().max(2e3).optional().default(""),
  location: z.string().trim().max(300).optional().default(""),
  image_url: z.string().trim().max(1e3).optional().default("")
});
createServerFn({
  method: "POST"
...
const submitAdComment = createServerFn({
  ...
  body: z.string().trim().min(1).max(1e3)
}).parse(d)).handler(createSsrRpc("1ce867041959958fcb72da9a77d8774d8c5933420d2de303a25279903543e4fc"));
const $$splitComponentImporter$k = () => import("./ads._adId-Bk_o9r7P.js");
const $$splitNotFoundComponentImporter = () => import("./ads._adId-CVQVz34c.js");
const $$splitErrorComponentImporter = () => import("./ads._adId-DlCU7388.js");
const Route$t = createFileRoute("/ads/$adId")({
  loader: async ({
    params
  }) => {
...
const Route$s = createFileRoute("/email/unsubscribe")({
  server: {
    handlers: {
      GET: async ({ request }) => {
...
const $$splitComponentImporter$j = () => import("./project._id-CosGpjmk.js");
const Route$r = createFileRoute("/project/$id")({
  validateSearch: (search) => ({
...
const $$splitComponentImporter$i = () => import("./vip.index-CnlUK5iu.js");
const Route$q = createFileRoute("/vip/")({
  head: () => ({
    meta: [{
...
const Route$p = createFileRoute("/_authenticated/admin/")({
  beforeLoad: () => {
    throw redirect({ to: "/admin/projects" });
  }
});
const $$splitComponentImporter$h = () => import("./admin.ads-DD8EnjSm.js");
const Route$o = createFileRoute("/_authenticated/admin/ads")({
  component: lazyRouteComponent($$splitComponentImporter$h, "component")
});
const $$splitComponentImporter$g = () => import("./admin.bot-settings-lGEztzts.js");
const Route$n = createFileRoute("/_authenticated/admin/bot-settings")({
  component: lazyRouteComponent($$splitComponentImporter$g, "component")
});
const $$splitComponentImporter$f = () => import("./admin.bot-test-BlRI_5yP.js");
const Route$m = createFileRoute("/_authenticated/admin/bot-test")({
  component: lazyRouteComponent($$splitComponentImporter$f, "component")
});
const $$splitComponentImporter$e = () => import("./admin.bot-training-8LCA14Sc.js");
const Route$l = createFileRoute("/_authenticated/admin/bot-training")({
  component: lazyRouteComponent($$splitComponentImporter$e, "component")
});
const $$splitComponentImporter$d = () => import("./admin.chat-DcVUoNgY.js");
const Route$k = createFileRoute("/_authenticated/admin/chat")({
  component: lazyRouteComponent($$splitComponentImporter$d, "component")
});
const $$splitComponentImporter$c = () => import("./admin.employees-CTE-cMVN.js");
const Route$j = createFileRoute("/_authenticated/admin/employees")({
  component: lazyRouteComponent($$splitComponentImporter$c, "component")
});
const $$splitComponentImporter$b = () => import("./admin.exclusivity-CpgHnH_f.js");
const Route$i = createFileRoute("/_authenticated/admin/exclusivity")({
  component: lazyRouteComponent($$splitComponentImporter$b, "component")
});
const $$splitComponentImporter$a = () => import("./admin.groq-settings-BkmPI05z.js");
const Route$h = createFileRoute("/_authenticated/admin/groq-settings")({
  component: lazyRouteComponent($$splitComponentImporter$a, "component")
});
const $$splitComponentImporter$9 = () => import("./admin.messages-BxsLAN3r.js");
const Route$g = createFileRoute("/_authenticated/admin/messages")({
  component: lazyRouteComponent($$splitComponentImporter$9, "component")
});
const $$splitComponentImporter$8 = () => import("./admin.my-projects-BLuQrTo2.js");
const Route$f = createFileRoute("/_authenticated/admin/my-projects")({
  component: lazyRouteComponent($$splitComponentImporter$8, "component")
});
const $$splitComponentImporter$7 = () => import("./admin.offers-BYCSyR6A.js");
const Route$e = createFileRoute("/_authenticated/admin/offers")({
  head: () => ({
    meta: [{
...
const $$splitComponentImporter$6 = () => import("./admin.pending-projects-CT3DLD0k.js");
const Route$d = createFileRoute("/_authenticated/admin/pending-projects")({
  component: lazyRouteComponent($$splitComponentImporter$6, "component")
});
const $$splitComponentImporter$5 = () => import("./admin.projects-uFsBk4P0.js");
const Route$c = createFileRoute("/_authenticated/admin/projects")({
  component: lazyRouteComponent($$splitComponentImporter$5, "component")
});
const $$splitComponentImporter$4 = () => import("./admin.requests-Dcxbsrtp.js");
const Route$b = createFileRoute("/_authenticated/admin/requests")({
  component: lazyRouteComponent($$splitComponentImporter$4, "component")
});
const $$splitComponentImporter$3 = () => import("./admin.settings-Da8K9Xfp.js");
const Route$a = createFileRoute("/_authenticated/admin/settings")({
  component: lazyRouteComponent($$splitComponentImporter$3, "component")
});
const $$splitComponentImporter$2 = () => import("./admin.support-B_v5NRt1.js");
const Route$9 = createFileRoute("/_authenticated/admin/support")({
  component: lazyRouteComponent($$splitComponentImporter$2, "component")
});
const $$splitComponentImporter$1 = () => import("./admin.users-CQORcohv.js");
const Route$8 = createFileRoute("/_authenticated/admin/users")({
  component: lazyRouteComponent($$splitComponentImporter$1, "component")
});
const $$splitComponentImporter = () => import("./admin.vip-BQKqiLPx.js");
const Route$7 = createFileRoute("/_authenticated/admin/vip")({
...
const Route$6 = createFileRoute("/api/admin/r2")({
  server: {
    handlers: {
...
const Route$5 = createFileRoute("/api/cron/vip-expiry")({
  server: {
    handlers: {
      GET: async ({ request }) => {
...
const Route$4 = createFileRoute("/api/public/upload")({
  server: {
    handlers: {
...
function RequestAcceptedEmail({ requestId = "", companyName = "" }) {
  return /* @__PURE__ */ jsxs(Html, { lang: "ar", dir: "rtl", children: [
    /* @__PURE__ */ jsx(Head, {}),
    /* @__PURE__ */ jsx(Preview, { children: "طلبك مقبول" }),
    /* @__PURE__ */ jsx(Body, { style: { fontFamily: "Cairo, Arial, sans-serif", background: "#f6f7f9", padding: "24px" }, children: /* @__PURE__ */ jsxs(Container, { style: { background: "#fff", borderRadius: 8, padding: 24, maxWidth: 560 }, children: [
      /* @__PURE__ */ jsx(Heading, { style: { fontSize: 20, margin: 0, color: "#111" }, children: "طلبك مقبول" }),
      companyName ? /* @__PURE__ */ jsxs(Text, { style: { color: "#444", marginTop: 12 }, children: [
        "مرحباً ",
        companyName,
        "،"
      ] }) : null,
      /* @__PURE__ */ jsxs(Text, { style: { color: "#222", fontSize: 16 }, children: [
        "طلبك رقم ",
        /* @__PURE__ */ jsx("strong", { children: requestId }),
        " تم قبوله."
      ] }),
      /* @__PURE__ */ jsx(Text, { style: { color: "#666", fontSize: 13, marginTop: 24 }, children: "سيتم التواصل معكم قريباً لاستكمال الإجراءات." })
    ] }) })
  ] });
}
const template = {
...
const Route = createFileRoute("/lovable/email/transactional/send")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const apiKey = process.env.LOVABLE_API_KEY;
...
function DefaultErrorComponent({ error, reset }) {
  ...
  return /* @__PURE__ */ jsx("div", { className: "flex min-h-screen items-center justify-center bg-background px-4", dir: "rtl", children: /* @__PURE__ */ jsxs("div", { className: "max-w-md text-center", children: [
    /* @__PURE__ */ jsx("h1", { className: "text-xl font-semibold tracking-tight text-foreground", children: "تعذر تحميل الصفحة" }),
    /* @__PURE__ */ jsx("p", { className: "mt-2 text-sm text-muted-foreground", children: "حدث خطأ مؤقت. جرّب تحديث الصفحة أو العودة للرئيسية." }),
    /* @__PURE__ */ jsxs("div", { className: "mt-6 flex flex-wrap justify-center gap-2", children: [
      /* @__PURE__ */ jsx(
        "button",
        {
