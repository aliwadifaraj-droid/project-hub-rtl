...
import { getCerebrasSettings, updateCerebrasSettings, type CerebrasSettings } from "@/lib/bot-settings.functions";

export const Route = createFileRoute("/_authenticated/admin/cerebras-settings")({
  component: CerebrasSettingsPage,
});

function CerebrasSettingsPage() {
  const qc = useQueryClient();
  const fetchCfg = useServerFn(getCerebrasSettings);
  const saveCfg = useServerFn(updateCerebrasSettings);
  const { data, isLoading } = useQuery({ queryKey: ["cerebras-cfg"], queryFn: () => fetchCfg() });
  ...
  useEffect(() => {
    ...
    setScope(data.scope);
    setCerebrasEnabled(data.cerebrasEnabled ?? true);
  }, [data]);

  async function save() {
    setSaving(true);
    try {
      const payload: CerebrasSettings = { systemInstruction, dialect, botName, blockedReplies: blocked, scope, cerebrasEnabled };
      await saveCfg({ data: payload });
      qc.invalidateQueries({ queryKey: ["cerebras-cfg"] });
      toast.success("تم حفظ إعدادات Cerebras");
    } catch (e: any) {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="mb-4 flex items-center gap-2">
        ...
        <div>
          <h1 className="text-xl font-bold">إعدادات Cerebras</h1>
        ...
        <div className="space-y-4">
          <section className="rounded-xl border border-border bg-background p-4 shadow-sm">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-sm font-bold">تفعيل Cerebras</h2>
                <p className="text-xs text-muted-foreground">تشغيل أو إيقاف ردود Cerebras للأسئلة الخارجية</p>
              </div>
              <button
                type="button"
                onClick={() => setCerebrasEnabled(!cerebrasEnabled)}
                role="switch"
                aria-checked={cerebrasEnabled}
                className={`relative h-6 w-11 rounded-full transition ${cerebrasEnabled ? "bg-primary" : "bg-muted"}`}
              >
                <span
