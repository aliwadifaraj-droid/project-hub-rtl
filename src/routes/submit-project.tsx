...
import { submitVisitorProject } from "@/lib/public.functions";
...
export const Route = createFileRoute("/submit-project")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "أرسل مشروعك ليتم نشره مباشرة على المنصة." },
    ],
  }),
...
function SubmitProjectPage() {
  const submitProject = useServerFn(submitVisitorProject);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!result?.id) {
        throw new Error("لم يتم حفظ المشروع في قاعدة البيانات");
      }
      setDone(true);
    } catch (err) {
        ...
        <div className="mx-auto max-w-2xl rounded-2xl border border-border bg-card p-6 md:p-10 shadow-[var(--shadow-card)]">
          {done ? (
            <div className="text-center py-8">
              ...
              <h1 className="mt-4 text-2xl font-bold">تم نشر مشروعك بنجاح</h1>
              <p className="mt-2 text-muted-foreground">مشروعك أصبح ظاهراً الآن في الصفحة الرئيسية.</p>
              <Link
                to="/"
            ...
            <>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                املأ التفاصيل وسيتم نشر مشروعك مباشرة على المنصة.
              </p>
              <form onSubmit={onSubmit} className="mt-6 space-y-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <Field label="المدة المتوقعة (اختياري)">
                    <input
                      maxLength={100}
