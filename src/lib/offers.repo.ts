...
export async function insertOffer(o: OfferInsert): Promise<string> {
  ...
  await db.execute(
    `INSERT INTO offers (id, project_id, project_name, company_name, email, amount, duration, pdf_key, pdf_filename, status, visitor_token, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [id, o.project_id ?? null, o.project_name, o.company_name, o.email, o.amount, o.duration ?? null, o.pdf_key ?? null, o.pdf_filename ?? null, o.status ?? "new", o.visitor_token ?? null, new Date().toISOString()],
  );
  return id;
}
...
export async function findProjectForOffer(name: string): Promise<{ id: string; name: string; duration: string | null; offers_enabled: boolean; bot_offers_enabled: boolean } | null> {
  ...
  return row ? { id: String(row.id), name: String(row.name), duration: row.duration ?? null, offers_enabled: Number(row.offers_enabled ?? 1) !== 0, bot_offers_enabled: Number(row.bot_offers_enabled ?? 1) !== 0 } : null;
}
...
export async function searchOffersByEmail(email: string): Promise<OfferRow[]> {
  const r = await db.execute(`SELECT * FROM offers WHERE lower(email) = lower(?) ORDER BY created_at DESC LIMIT 10`, [email.trim()]);
  return rowsToObjects(r).map(decode);
}

export async function searchOffersByCompany(q: string): Promise<OfferRow[]> {
  const r = await db.execute(`SELECT * FROM offers WHERE company_name LIKE ? ORDER BY created_at DESC LIMIT 10`, [`%${q}%`]);
  return rowsToObjects(r).map(decode);
}

export async function existsDuplicateOffer(projectName: string, email: string, companyName: string): Promise<boolean> {
  const p = (projectName ?? "").trim().toLowerCase();
  const e = (email ?? "").trim().toLowerCase();
  const c = (companyName ?? "").trim().toLowerCase();
  const o = await db.execute(`SELECT 1 FROM offers WHERE LOWER(TRIM(project_name)) = ? AND (LOWER(TRIM(email)) = ? OR LOWER(TRIM(company_name)) = ?) LIMIT 1`, [p, e, c]);
  if (rowsToObjects(o).length > 0) return true;
  try {
    const r = await db.execute(`SELECT 1 FROM project_requests WHERE LOWER(TRIM(COALESCE(facility_location,''))) = ? AND (LOWER(TRIM(COALESCE(email,''))) = ? OR LOWER(TRIM(COALESCE(company_name,''))) = ?) LIMIT 1`, [p, e, c]);
