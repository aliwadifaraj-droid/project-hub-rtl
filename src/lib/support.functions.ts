...
async function answerProjectQuery(admin: any, text: string): Promise<string | null> {
  const t = (text ?? "").toLowerCase();
  const mentionsProject = t.includes("مشروع") || t.includes("المشروع");
  const mentionsProjectsList = LIST_KEYWORDS.some((k) => t.includes(k));
  ...
  // 2) Specific project name query → return ONLY: name + location + description + status
  if (mentionsProject) {
    ...
    if (tokens.length) {
      ...
      // Project name mentioned but not found → no generic / Groq fallback
      return "ما لقيت مشروع بهذا الاسم في المنصة. تأكد من الاسم أو اطلب قائمة المشاريع.";
    }
    // "مشروع" with no specific name → fall through to list
  }

  // 3) General projects list question → short list: name + location + status
  if (mentionsProjectsList) {
    const { data } = await admin
      .from("projects")
