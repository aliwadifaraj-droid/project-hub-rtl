    ...
    `SELECT v.*,
       (SELECT p.name FROM projects p
        WHERE p.location IS NOT NULL
          AND TRIM(LOWER(v.city)) = TRIM(LOWER(
            CASE
              WHEN INSTR(p.location, '-') > 0
              THEN SUBSTR(p.location, 1, INSTR(p.location, '-') - 1)
              ELSE p.location
            END
          ))
          AND EXISTS (
            SELECT 1 FROM project_exclusive pe
            WHERE pe.project_id = p.id
            AND datetime(pe.vip_end_at) > datetime('now')
          )
        LIMIT 1
       ) AS project_name
...
export async function cancelByProject(projectId: string): Promise<void> {
  ...
  await setProjectVip(projectId, false, 0);
}
...
export async function setProjectVip(projectId: string, isVip: boolean, days: number) {
  const end = isVip? new Date(Date.now() + days * 86400_000).toISOString() : null;
  await db.execute(`UPDATE projects SET is_vip =?, vip_end_at =? WHERE id =?`, [isVip? 1 : 0, end, projectId]);
}
