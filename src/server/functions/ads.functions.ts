...
export const createAd = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await supabaseAdmin.from("project_submissions").update({ link_url: linkUrl }).eq("id", row.id);
...
export const deleteAd = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await assertAdmin(supabase, userId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { error } = await supabaseAdmin.from("project_submissions").delete().eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
