...
import { existsDuplicateOfferNotification, insertOfferNotificationMany, existsDuplicateAddProjectNotification, listAllOfferNotifications, countNewOfferNotifications, getOfferNotificationById, deleteOfferNotification, updateOfferNotificationStatus } from "./notifications.repo-CpR-n2ns.mjs";
...
import { g as getByNameExact, a as getProjectExclusive } from "./projects.repo-GgbtJXPt.mjs";
import { d as detectCity } from "./vip-notify.server-PhbhwAkL.mjs";
...
const OFFER_EXCLUSIVE_MESSAGE = "هذا المشروع في فترة الحصرية";
const OFFER_PROJECT_NOT_FOUND_MESSAGE = "المشروع غير موجود";
...
const submitOffer = createServerFn({
  ...
  const project = await getByNameExact(data.projectName).catch(() => null);
  if (!project) {
    return {
      ok: false,
      message: OFFER_PROJECT_NOT_FOUND_MESSAGE
    };
  }
  const exclusive = await getProjectExclusive(project.id).catch(() => null);
  if (exclusive && Date.now() < new Date(exclusive.vip_end_at).getTime()) {
    ...
    if (!hasVipAccess) {
      ...
      if (email) {
        const vip = await (await import("./vip.repo-BoiBu0-3.mjs")).getActiveVipByEmail(email);
        const projectCity = detectCity(project.location ?? "");
        const vipCity = vip?.city?.trim().toLowerCase();
        hasVipAccess = !!vip && (!vip.expires_at || new Date(vip.expires_at).getTime() >= Date.now()) && !!projectCity && !!vipCity && projectCity.trim().toLowerCase() === vipCity;
      }
    }
    if (!hasVipAccess) {
      return {
        ok: false,
        message: OFFER_EXCLUSIVE_MESSAGE
      };
    }
  }
  ...
  if (data.visitorToken) {
    try {
      const support = await import("./support.repo-DXE-PmER.mjs");
...
const adminUpdateOfferStatus = createServerFn({
  ...
  if (data.status === "accepted") {
    ...
    const requests = await import("./project-requests.repo-tu1FwgzE.mjs");
