...
import { a as requireAdmin, r as requireAuth } from "./auth-middleware.server-CWyFWbOs.mjs";
import { q as listPending, r as countPending, b as getById, u as updateProject } from "./projects.repo-GgbtJXPt.mjs";
...
import { autoActivateByCity } from "./vip.repo-BoiBu0-3.mjs";
import { n as notifyVipSubscribersOfNewProject } from "./vip-notify.server-PhbhwAkL.mjs";
import { d as db } from "./db-BSVZwhof.mjs";
import { setExclusiveWindow } from "./project-exclusive.repo-DD5-5Rm0.mjs";
...
async function applyExclusiveWindow(projectId, location, city) {
  const { detectCity } = await import("./vip-notify.server-PhbhwAkL.mjs").then((n) => n.a);
  const detected = detectCity(location);
  if (!detected) return;
  const { listActiveByCity } = await import("./vip.repo-BoiBu0-3.mjs");
...
const approveProject = createServerFn({
  ...
  if (row.created_by) {
    const {
      insertOne
    } = await import("./notifications.repo-CpR-n2ns.mjs");
    await insertOne({
