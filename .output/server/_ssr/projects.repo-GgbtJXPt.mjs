...
async function getByNameExact(name) {
  const r = await db.execute(
...
async function updateProjectExclusivity(projectId, durationHours) {
  ...
  const { setExclusiveWindow } = await import("./project-exclusive.repo-DD5-5Rm0.mjs");
...
async function getProjectExclusive(projectId) {
  const { getExclusiveWindow } = await import("./project-exclusive.repo-DD5-5Rm0.mjs");
  const w = await getExclusiveWindow(projectId);
  if (!w) return null;
  return { vip_start_at: w.vip_start_at, vip_end_at: w.vip_end_at };
}
export {
  getProjectExclusive as a,
  getById as b,
  setBotOffersEnabled as c,
  deleteProject as d,
  setAllBotOffersEnabled as e,
  findByOwnerAndName as f,
  getByNameExact as g,
  setOffersEnabled as h,
  insertProject as i,
  setAllOffersEnabled as j,
  updateProjectExclusivity as k,
  listAllProjects as l,
  searchByName as m,
  listByOwner as n,
  findByAdId as o,
  setExclusive as p,
